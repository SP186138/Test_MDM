...
.GOTO $Step
 ...
 CREATE SET TABLE USER_WORK.REGIS_PRSNA_DUP1 ,NO FALLBACK ,
     NO BEFORE JOURNAL,
     NO AFTER JOURNAL,
     CHECKSUM = DEFAULT,
...
CREATE SET TABLE USER_WORK.REGIS_PRSNA_DUP2 ,NO FALLBACK ,
     NO BEFORE JOURNAL,
     NO AFTER JOURNAL,
     CHECKSUM = DEFAULT,
     DEFAULT MERGEBLOCKRATIO
...
INSERT INTO USER_WORK.REGIS_PRSNA_DUP1(REGIS_PRSNA_ID
   ,MKTNG_PGM_NBR
   ,LEGAL_ENT_NBR
...
WHERE CNTRY_CODE IN ('LAT','CAN','ARG','CHL','MEX')
GROUP BY 1
HAVING COUNT (REGIS_PRSNA_ID) > 1);
...
INSERT INTO USER_WORK.REGIS_PRSNA_DUP2
SEL * FROM USER_WORK.REGIS_PRSNA_DUP1
QUALIFY RANK() OVER(PARTITION BY REGIS_PRSNA_ID,MATCHD_CNSMR_PRSNA_ID, LEGAL_ENT_NBR,MKTNG_PGM_NBR ORDER BY
PRSNA_STATUS_CODE ASC,GVN_NAME_ALIAS ASC,FAMLY_NAME_ALIAS ASC)=1;

.IF ERRORLEVEL > 0 THEN .QUIT ERRORCODE;

.LABEL L4

DEL  FROM MDM.REGIS_PRSNA WHERE REGIS_PRSNA_ID IN (SEL DISTINCT REGIS_PRSNA_ID FROM USER_WORK.REGIS_PRSNA_DUP1);
...
SEL  REGIS_PRSNA_ID
   ...
   ,PII_DATA_MINIMIZE_DATETM
   ,GVN_NAME_ALIAS
   ,FAMLY_NAME_ALIAS FROM USER_WORK.REGIS_PRSNA_DUP2;
...
SET VALID_CNTCT_POINT_IND='Y'	
,SYS_LAST_MODIFIED_DATE=CURRENT_DATE
,SYS_LAST_MODIFIED_BY='CHG0032903'
,SYS_ERR_CODE='R552'
WHERE MKTNG_PGM_NBR IN (SEL MKTNG_PGM_NBR FROM MKTNG_PGM WHERE LEGAL_ENT_NBR IN (23))					
AND VALID_CNTCT_POINT_IND='N'		
AND PR_GEOCODE_FAIL='0'
AND PR_REV_GROUP='020'
;
.IF ERRORLEVEL > 0 THEN .QUIT ERRORCODE;
