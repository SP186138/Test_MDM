...
.GOTO $Step
 ...
 DEL FROM TRILLIUM_OUTPUT1_ESP_TEMP;

 .IF ERRORLEVEL > 0 THEN .QUIT ERRORCODE;
 .LABEL L1

INS TRILLIUM_OUTPUT1_ESP_TEMP
(	   MKTNG_PGM_NBR
      ,REGIS_CNSMR_ID_VAL
      ,DR_ALIAS_CONTACT
      ,DR_FIRST_NAME
      ...
      ,GL_LONGITUDE
      ,DR_CLEANSE_LEVEL
	  FROM TRILLIUM_OUTPUT1_ESP;

 .IF ERRORLEVEL > 0 THEN .QUIT ERRORCODE;
 .LABEL L2

DROP TABLE REGIS_PRSNA_TEMP_ESP;
...
INSERT INTO REGIS_PRSNA_TEMP_ESP
(MKTNG_PGM_NBR ,LEGAL_ENT_NBR,REGIS_CNSMR_ID_VAL,SYS_TARGET_ID,REGIS_PRSNA_ID)
SELECT
MKTNG_PGM_NBR ,LEGAL_ENT_NBR,REGIS_CNSMR_ID_VAL,SYS_TARGET_ID,REGIS_PRSNA_ID
FROM REGIS_PRSNA
WHERE MKTNG_PGM_NBR IN (SEL MKTNG_PGM_NBR FROM TRILLIUM_OUTPUT1_ESP_TEMP);

 .IF ERRORLEVEL > 0 THEN .QUIT ERRORCODE;
 .LABEL L4

UPDATE TRILLIUM_OUTPUT1_ESP_TEMP SET RECORD_IND='Y'    ---  records matching REGIS_PRSNA
WHERE EXISTS
(SELECT
1
FROM
REGIS_PRSNA_TEMP_ESP
WHERE
LEGAL_ENT_NBR = TRILLIUM_OUTPUT1_ESP_TEMP.LEGAL_ENT_NBR
AND MKTNG_PGM_NBR = TRILLIUM_OUTPUT1_ESP_TEMP.MKTNG_PGM_NBR   
AND REGIS_CNSMR_ID_VAL = TRILLIUM_OUTPUT1_ESP_TEMP.REGIS_CNSMR_ID_VAL
AND SYS_TARGET_ID = TRILLIUM_OUTPUT1_ESP_TEMP.SYS_TARGET_ID
);
...
UPDATE TRILLIUM_OUTPUT1_ESP_TEMP
FROM
(
SELECT * FROM
(
...
,SYS_NC_TYPE
,SYS_TARGET_ID
FROM TRILLIUM_OUTPUT1_ESP_TEMP
) B
WHERE RNK = 1
) B

SET RECENT_IND = 'Y'
WHERE TRILLIUM_OUTPUT1_ESP_TEMP.LEGAL_ENT_NBR=B.LEGAL_ENT_NBR
AND TRILLIUM_OUTPUT1_ESP_TEMP.MKTNG_PGM_NBR=B.MKTNG_PGM_NBR
AND TRILLIUM_OUTPUT1_ESP_TEMP.REGIS_CNSMR_ID_VAL=B.REGIS_CNSMR_ID_VAL
AND TRILLIUM_OUTPUT1_ESP_TEMP.REGIS_DATE=B.REGIS_DATE
AND TRILLIUM_OUTPUT1_ESP_TEMP.SYS_CREATION_DATE=B.SYS_CREATION_DATE
AND TRILLIUM_OUTPUT1_ESP_TEMP.SYS_NC_TYPE=B.SYS_NC_TYPE
AND TRILLIUM_OUTPUT1_ESP_TEMP.SYS_SOURCE=B.SYS_SOURCE
AND TRILLIUM_OUTPUT1_ESP_TEMP.SYS_TARGET_ID=B.SYS_TARGET_ID
;
...
FROM TRILLIUM_OUTPUT1_ESP_TEMP A
...
WHERE TRILLIUM_OUTPUT1_ESP_TEMP.MKTNG_PGM_NBR = B.MKTNG_PGM_NBR
AND TRILLIUM_OUTPUT1_ESP_TEMP.REGIS_CNSMR_ID_VAL = B.REGIS_CNSMR_ID_VAL
AND TRILLIUM_OUTPUT1_ESP_TEMP.LEGAL_ENT_NBR = B.LEGAL_ENT_NBR
AND TRILLIUM_OUTPUT1_ESP_TEMP.SYS_SOURCE = B.SYS_SOURCE
AND TRILLIUM_OUTPUT1_ESP_TEMP.RECENT_IND = 'Y'
;
...
INNER JOIN TRILLIUM_OUTPUT1_ESP_TEMP B
...
WHERE TRILLIUM_OUTPUT1_ESP_TEMP.REFERENCE_HOUSEHOLD_ID = B.REFERENCE_HOUSEHOLD_ID
AND TRILLIUM_OUTPUT1_ESP_TEMP.LEGAL_ENT_NBR = B.SYS_TARGET_ID;
...
WHERE TRILLIUM_OUTPUT1_ESP_TEMP.REFERENCE_LEGALENTITYKEY = B.REFERENCE_LEGALENTITYKEY
AND TRILLIUM_OUTPUT1_ESP_TEMP.LEGAL_ENT_NBR = B.SYS_TARGET_ID;
...
COLLECT STATS  TRILLIUM_OUTPUT1_ESP_TEMP
COLUMN HOUSEHOLD_ID;
 .IF ERRORLEVEL > 0 THEN .QUIT ERRORCODE;
 .LABEL L14
...
DEL FROM TSS_PRSNA_IDS_ESP ALL;
																					
 .IF ERRORLEVEL > 0 THEN .QUIT ERRORCODE;
 .LABEL L24


INSERT INTO TSS_PRSNA_IDS_ESP
(
REGIS_CNSMR_ID_VAL,
MKTNG_PGM_NBR,
LEGAL_ENT_NBR,
...
DEL FROM TSS_PRSNA_IDS_ESP_PREV ALL;
																					
 .IF ERRORLEVEL > 0 THEN .QUIT ERRORCODE;
 .LABEL L26


INSERT INTO TSS_PRSNA_IDS_ESP_PREV
(
REGIS_CNSMR_ID_VAL,
MKTNG_PGM_NBR,
LEGAL_ENT_NBR,
...
(SEL MKTNG_PGM_NBR,REGIS_PRSNA_ID FROM TSS_PRSNA_IDS_ESP)
;

 .IF ERRORLEVEL > 0 THEN .QUIT ERRORCODE;
 .LABEL L27

COLLECT STATS TSS_PRSNA_IDS_ESP
...
REFERENCE_LEGALENTITYKEY,NATIONAL_ID_NBR
...
= count(1)over(partition by LEGAL_ENT_NBR,MKTNG_PGM_NBR,REGIS_PRSNA_ID,SYS_TARGET_ID,REFERENCE_LEGALENTITYKEY,NATIONAL_ID_NBR,COALESCE(NULLIF(TRIM(DR_ALIAS_CONTACT),''),DR_FIRST_NAME),COALESCE(NULLIF(TRIM(DR_ALIAS_ACCOUNT),''),DR_LAST_NAME))
) B
SET MATCHD_CNSMR_PRSNA_ID=CAST(B.REFERENCE_LEGALENTITYKEY AS INTEGER)
, NATIONAL_ID_NBR=B.NATIONAL_ID_NBR
...
--Updating the Postal Address Table with the Winkey Value of the Trillium Output Table
--We are not going to change anything in the Postal Address
/*
DEL FROM ADDR_TEMP;
...
REGISTRATIONKEY   
FROM TRILLIUM_OUTPUT1_ESP_TEMP
WHERE ADDRESS_CONTACT IS NOT NULL;
...
WHEN (GOUT_MATCH_LEVEL IN ('0','9')
      ...
      'SGP','USA','CAN','BRA','MEX','FRA','BEL','NLD','ITA','GRC','AUT','CHE','ESP','ESP','IRL'
      ...
      AND DR_COUNTRY_NAME IN ('CHE','ESP','ESP','BEL','NLD'))
...
QUALIFY RANK() OVER (PARTITION BY TOUT1.REGIS_PRSNA_ID,TOUT.ADDRESS_CONTACT
ORDER BY TOUT.DR_ADDRESS DESC
...
AND (A. MKTNG_PGM_NBR,B.REGIS_PRSNA_ID) IN (SEL MKTNG_PGM_NBR,REGIS_PRSNA_ID FROM TSS_PRSNA_IDS_ESP)
QUALIFY RANK() OVER (PARTITION BY B.MATCHD_CNSMR_PRSNA_ID,
CNTCT_POINT_CATEG_CODE,
...
(
SEL MATCHD_CNSMR_PRSNA_ID FROM TSS_PRSNA_IDS_ESP
UNION
SEL MATCHD_CNSMR_PRSNA_ID FROM TSS_PRSNA_IDS_ESP_PREV
)
WITH DATA
...
WHERE RP.MKTNG_PGM_NBR IN (SEL MKTNG_PGM_NBR FROM TSS_PRSNA_IDS_ESP)
...
AND REGIS_PRSNA_POSTL_ADDR.MKTNG_PGM_NBR IN (SEL MKTNG_PGM_NBR FROM TRILLIUM_OUTPUT1_ESP_TEMP)
...
AND REGIS_PRSNA_PHONE.MKTNG_PGM_NBR IN (SEL MKTNG_PGM_NBR FROM TRILLIUM_OUTPUT1_ESP_TEMP)
...
AND REGIS_PRSNA_EMAIL_ADDR.MKTNG_PGM_NBR IN (SEL MKTNG_PGM_NBR FROM TRILLIUM_OUTPUT1_ESP_TEMP)
...
AND REGIS_PRSNA_SOC_NET_ACCT.MKTNG_PGM_NBR IN (SEL MKTNG_PGM_NBR FROM TRILLIUM_OUTPUT1_ESP_TEMP)
...
AND REGIS_PRSNA.MKTNG_PGM_NBR = B.MKTNG_PGM_NBR
AND REGIS_PRSNA.MKTNG_PGM_NBR IN (SEL MKTNG_PGM_NBR FROM TRILLIUM_OUTPUT1_ESP_TEMP)
