...
(SELECT MATCHD_CNSMR_PRSNA_ID,LEGAL_ENT_NBR 
FROM MDM.MATCH_DATA_TEMP1) AND LEGAL_ENT_NBR IN (23);
 .IF ERRORLEVEL > 0 THEN .QUIT ERRORCODE;
 .LABEL L2
...
FROM ICRM_TBL.MATCHD_CNSMR_PRSNA) AND LEGAL_ENT_NBR IN (23);
...
AND A.MKTNG_PGM_NBR IN (SEL MKTNG_PGM_NBR FROM MKTNG_PGM  WHERE LEGAL_ENT_NBR =23)
AND B.MKTNG_PGM_NBR IN (SEL MKTNG_PGM_NBR FROM MKTNG_PGM  WHERE LEGAL_ENT_NBR =23)
