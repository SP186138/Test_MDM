...
CREATE SET TABLE DQRM_TBL.DQRM_METRIC_TABLE_LEVEL1_NEW ,FALLBACK ,
     ...
     (
      ...
      MARKET_SEGMENT VARCHAR(100) CHARACTER SET LATIN NOT CASESPECIFIC,
...
RENAME TABLE DQRM_TBL.DQRM_METRIC_TABLE_LEVEL1 TO DQRM_TBL.DQRM_METRIC_TABLE_LEVEL1_R582_Rollback ;
...
ON CASE WHEN TRIM(LEGAL_ENTITY) BETWEEN '00000' AND '32767' THEN CAST(LEGAL_ENTITY AS SMALLINT) ELSE 0 END  = L.LEGAL_ENT_NBR
LEFT OUTER JOIN ICRM.MKTNG_PGM M
ON CASE WHEN TRIM(MARKET_SEGMENT) BETWEEN '000000000' AND '2147483647' THEN CAST(MARKET_SEGMENT AS INTEGER) ELSE 0 END  = M.MKTNG_PGM_NBR
LEFT OUTER JOIN (
SELECT	DATA_SRCE_NBR, MAX(DATA_SRCE_NAME) DATA_SRCE_NAME
FROM ICRM.DATA_SRCE
GROUP BY 1
)D
ON CASE WHEN TRIM(SOURCE_SYSTEM) BETWEEN '000000000' AND '2147483647' THEN CAST(SOURCE_SYSTEM AS INTEGER) ELSE 0 END  = D.DATA_SRCE_NBR
LEFT OUTER JOIN 
...
AND R.RULE_REVISION = DQR.RULE_REVISION
LEFT OUTER JOIN ICRM.LEGAL_ENT L
ON CASE WHEN TRIM(R.LEGAL_ENTITY) BETWEEN '00000' AND '32767' THEN CAST(R.LEGAL_ENTITY AS SMALLINT) ELSE 0 END  = L.LEGAL_ENT_NBR;
